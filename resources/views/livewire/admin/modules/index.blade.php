<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <x-slot:rows>
            @forelse ($modules as $module)
                <tr wire:key="module-{{ $module->id }}">
                    ...
                    <td class="px-4 py-2">
                        <flux:badge :color="$module->is_active ? 'success' : 'neutral'">
                        ...
                        <div class="flex items-center gap-2">
                            @can('update modules')
                                <flux:button size="xs" variant="ghost" icon="pencil-square"
                                    :href="route('admin.modules.edit', $module->id)" wire:navigate>{{ __('Edit') }}</flux:button>
                            @endcan
                            @can('delete modules')
                                <flux:button size="xs" variant="ghost" color="danger" icon="trash"
                                    wire:click="delete({{ $module->id }})">{{ __('Delete') }}</flux:button>
                            @endcan
                        </div>
                    </td>
                </tr>
    ...
    <flux:modal wire:model="showModal" focusable class="max-w-xl w-full">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $editingId ? __('Edit Module') : __('New Module') }}</flux:heading>
                <flux:subheading>{{ __('Manage application modules.') }}</flux:subheading>
            </div>
